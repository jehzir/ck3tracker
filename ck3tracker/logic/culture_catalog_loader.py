"""Load installed CK3 culture IDs with linked wiki provenance."""
...
@dataclass(frozen=True)
class CultureCatalogLoadResult:
    reference_snapshot_id: str
    culture_count: int
    parser_run_id: str


def load_culture_catalog_candidate(
    *,
    game_root: str | Path,
    reference_snapshot_id: str,
    wiki_root: WikiPageEvidence,
    wiki_culture: WikiPageEvidence,
    database_path: str | Path | None = None,
) -> CultureCatalogLoadResult:
    """Replace one candidate snapshot's installed culture catalog and wiki evidence."""
    if wiki_root.page_key != "wiki_root" or wiki_culture.page_key != "culture":
        raise ValueError("Expected wiki_root and culture page evidence")
    if wiki_culture.review_status != "reviewed":
        raise ValueError("Culture wiki evidence must be reviewed")

    root = Path(game_root).resolve()
    source_root = root / "common" / "culture" / "cultures"
    ...
    try:
        ...
        if snapshot[0] == "promoted":
            raise ValueError("A promoted culture catalog cannot be replaced")
        ...
        for page in (wiki_root, wiki_culture):
            connection.execute(
        ...
        connection.execute(
            "DELETE FROM source.wiki_page_links WHERE reference_snapshot_id = ? AND from_page_key = 'wiki_root' AND to_page_key = 'culture'",
            [reference_snapshot_id],
        )
        connection.execute(
            "INSERT INTO source.wiki_page_links VALUES (?, 'wiki_root', 'culture', 'Culture', ?)",
            [reference_snapshot_id, wiki_culture.canonical_url],
        )
        connection.executemany(
    ...
    return CultureCatalogLoadResult(reference_snapshot_id, len(rows), parser_run_id)